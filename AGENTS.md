...
14. **El `alias_token` es EFÍMERO: validez de UNA sola operación, TTL del orden de minutos (spec pág. 35, *"alias token temporal"*).** NUNCA cobres/borres con el alias guardado en `SavedCard`: se pide uno FRESCO a Bancard (`users_cards`) justo antes de cada charge/delete, matcheando la tarjeta por identidad ESTABLE (`card_id`, o masked+expiration). Lo hacen `chargeDefaultCard`/`chargeBancardCard`/`deleteBancardCard`; el delete es **idempotente** si la tarjeta ya no está en Bancard. `syncBancardCards()` deduplica por `card_id`, **NO** por `alias_token` (si no, cada sync duplicaría la tarjeta). El matching vive en **`BancardVPOSService::freshAliasToken($userId, $cardIdentity)`** (público) y el trait delega ahí — una sola fuente de verdad. **Consumidores que usan el service directo** (sin el trait) deben llamar ese helper antes de `chargeWithToken()`/`deleteCard()`, no cobrar/borrar con el alias guardado. Tests: `EphemeralAliasTest`, `test_fresh_alias_token_*`.
