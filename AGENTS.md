...
7. **3DS de charge — `extra_response_attributes` OBLIGATORIO**: el request de `chargeWithToken()` DEBE incluir `'extra_response_attributes' => ['confirmation.process_id']` (spec pág. 37, "Siempre enviar este dato"). Sin eso Bancard no devuelve `confirmation.process_id` y un cobro que exige 3DS cae en la rama *Payment rejected* en vez de `requires_3ds`. Hay test que lo bloquea (`test_charge_request_envia_extra_response_attributes_para_3ds`). El 3DS del **single_buy** NO requiere cambios de backend: es transparente (mismo `process_id`/`checkout_js_url`); en el frontend se usa `Bancard.Charge3DS.createForm` en vez de `Bancard.Checkout.createForm`.
