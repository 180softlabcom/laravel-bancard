...
> 🔑 **El `alias_token` es efímero** — Bancard lo describe como *"alias token temporal"*, con validez para **una sola operación** y TTL del orden de **minutos** (spec pág. 35). No hace falta que te preocupes por eso: `chargeDefaultCard`/`chargeBancardCard` y `deleteBancardCard` **piden un alias fresco a Bancard (`users_cards`) automáticamente** antes de cada operación, matcheando la tarjeta por su identidad estable (`card_id`). El `alias_token` guardado en `SavedCard` es solo una referencia; nunca cobres/borres con el guardado.
>
> **¿Usás el service de bajo nivel** (`BancardVPOSService`/`Bancard::`) **en vez del trait?** Pedí el alias fresco con el mismo helper antes de cada charge/delete:
> ```php
> $alias = $service->freshAliasToken($userId, ['card_id' => $cardId]); // o ['card_masked_number'=>…, 'expiration_date'=>…]
> if ($alias === null) { /* la tarjeta ya no está catastrada */ }
